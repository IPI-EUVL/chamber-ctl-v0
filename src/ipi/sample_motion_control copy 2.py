...
class SampleStageControl:
    ...
    def set_param(self):
        ...
        self.draw_interval = 0.01  #seconds between refresh of the GUI
    ...
    def widgetter(self):
        ...
        ttk.Button(manual_motion_frame, text="GOTO MANAL", command=None).grid(row=0, column=2, padx=5, sticky="ew")
        ...
        ttk.Button(manual_motion_frame, text="GOTO SAMPLE", command=None).grid(row=0, column=2, padx=5, sticky="ew")
    ...
    def vis_rot_update(self):
        if self.visual_rotation_active:
            self.visual_angle = (self.visual_angle + self.rotation_direction * 2)%360
            self.gui_safe(self.stager)
            self.root.after(20, self.vis_rot_update)

    def expos_cd_updater(self):
        if self.remaining_exposure_time > 0:
            self.exposure_countdown.config(text=f"Exposure Countdown: {self.remaining_exposure_time:.1f}s")
            self.remaining_exposure_time -= 0.1
            self.root.after(100, self.expos_cd_updater)
        else:
            self.exposure_countdown.config(text="Exposure Countdown: 0.0s")


    def update_gui_position(self):
        """Thread-safe GUI position update"""
        self.position_label.config(
            text=f"Position: {self.current_linear_pos:.2f}mm | Speed: {self.linspeedval} mm/s"
        )
        self.stager()
            
    def direct_signal_test(self):
        
        #Test window
        test_window = tk.Toplevel(self.root)
        test_window.title("Direct Signal Testing")
        
        #Linear motor controls
        ttk.Label(test_window, text="Linear Motor", font=('Segoe UI', 10, 'bold')).grid(row=0, column=0, columnspan=3, pady=5)
        
        #Enable/Disable
        ttk.Button(test_window, text="ENABLE", 
                command=lambda: ljm.eWriteName(self.handle, self.LIN_EN, 1)).grid(row=1, column=0, padx=5)
        ttk.Button(test_window, text="DISABLE", 
                command=lambda: ljm.eWriteName(self.handle, self.LIN_EN, 0)).grid(row=1, column=1, padx=5)
        
        #Direction control
        ttk.Button(test_window, text="DIR 5V", 
                command=lambda: ljm.eWriteName(self.handle, self.LIN_DIR, 5)).grid(row=2, column=0, padx=5)
        ttk.Button(test_window, text="DIR 0V", 
                command=lambda: ljm.eWriteName(self.handle, self.LIN_DIR, 2)).grid(row=2, column=1, padx=5)
        
        #Step pulse
        ttk.Button(test_window, text="SEND PULSE", 
                command=lambda: [
                    ljm.eWriteName(self.handle, self.LIN_STEP, 1),
                    time.sleep(0.001),
                    ljm.eWriteName(self.handle, self.LIN_STEP, 0)
                ]).grid(row=3, column=0, columnspan=2, pady=5)
        
        #Rotational motor controls
        ttk.Label(test_window, text="Rotational Motor", font=('Segoe UI', 10, 'bold')).grid(row=4, column=0, columnspan=3, pady=5)
        
        #Enable/Disable
        ttk.Button(test_window, text="ENABLE", 
                command=lambda: ljm.eWriteName(self.handle, self.ROT_EN, 1)).grid(row=5, column=0, padx=5)
        ttk.Button(test_window, text="DISABLE", 
                command=lambda: ljm.eWriteName(self.handle, self.ROT_EN, 0)).grid(row=5, column=1, padx=5)
        
        #Direction control
        ttk.Button(test_window, text="DIR HIGH", 
                command=lambda: ljm.eWriteName(self.handle, self.ROT_DIR, 1)).grid(row=6, column=0, padx=5)
        ttk.Button(test_window, text="DIR LOW", 
                command=lambda: ljm.eWriteName(self.handle, self.ROT_DIR, 0)).grid(row=6, column=1, padx=5)
        
        #Step pulse
        ttk.Button(test_window, text="SEND PULSE", 
                command=lambda: [
                    ljm.eWriteName(self.handle, self.ROT_PUL, 1),
                    time.sleep(0.001),
                    ljm.eWriteName(self.handle, self.ROT_PUL, 0)
                ]).grid(row=7, column=0, columnspan=2, pady=5)
        
        #Voltage check
        ttk.Button(test_window, text="CHECK VOLTAGES", 
                command=self.check_voltages).grid(row=8, column=0, columnspan=2, pady=10)

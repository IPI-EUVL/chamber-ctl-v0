...
def collect_raw_data():
        ...
        with open("C:\\Apps\\Testing\\Testfile.bin", "wb") as file:
            while running:
                try:
                    ...
                    print(f"Error",f"Error collecting raw data: {e}")
...
def load_and_plot_threshold_label_multi(file_paths,
        ...
        Args:
            ...
            threshold (float): Voltage thresholpython   d for pulse detection
...
##########################################################################################
            ...
            if not running:
                ...
                fig.update_layout( title=f"Oscilloscope Waveform: test", xaxis_title="Time (s)", yaxis_title="Voltage (V)", yaxis_range=[min(voltage_data.min(), -0.5), max(voltage_data.max(), 0.5) + 0.5], template="plotly_white", legend_title_text="Waveform Type" ) 
                fig.show()
...
def main():
        ...
        while running:
            ...
            load_and_plot_threshold_label_multi(
                file_paths="C:\\Apps\\Testing\\Testfile.bin",
                threshold=1,
                min_width_sec=7e-9,
                baseline_value=0.8,
                resistor_ohms=50,

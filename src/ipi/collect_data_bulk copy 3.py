...
def capture_burst_and_read(N=200):
    ...
    t, V, meta, timestamps = decode_sequence_waveforms(desc, data, 100)
...
def chopper_thread():
    global stop_flag, foldername
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock.bind(("1.1.1.1", 11750))

    chopper_fn = f"{foldername}\\chopper_{int(time.time())}.csv"
        ...
        try:
            while time.time() - start < 30.0 and not stop_flag:
                ...
                for nv in range(0, len(V), int(NUM_SKIP)):
                    ...
                    #print(" THE T", t)
                    #print("MYTIME: ", mytime)
                    values = np.column_stack((timestamps[nv], V[nv]))
                    #print("DARATA:", data)
